...
function AccueilTab({ profile, studios, studioClockOut, userId, onOpenNotifs, onGoFormation }: { profile: ProfileRow | null; studios: Record<string, string>; studioClockOut: Record<string, { before: number; grace: number }>; userId: string; onOpenNotifs: () => void; onGoFormation: () => void }) {
  ...
  // Note : la notification "changement de rôle dans 5 min" est envoyée
  // côté serveur par le cron `role-transitions-tick` → arrive dans la cloche
  // via realtime sur la table notifications.
...
function ProfilTab({ profile, businessRoles, studios, userId, onProfileChange, onNavigate }: { profile: ProfileRow | null; businessRoles: Role[]; studios: Record<string, string>; userId: string; onProfileChange: (patch: Partial<ProfileRow>) => void; onNavigate: (t: Tab) => void }) {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const params = new URLSearchParams(window.location.search);
    if (params.get("openDocs") === "1") {
      setDocsOpen(true);
      params.delete("openDocs");
