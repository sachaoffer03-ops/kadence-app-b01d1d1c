...
BEGIN
  ...
  -- Ponctualité : uniquement les shifts réellement pointés.
  -- Un shift passé sans pointage est ignoré (et non compté 0).
  WITH last_shifts AS (
  ...
  -- Check-lists : uniquement les shifts où une soumission existe.
  -- Un shift sans check-list est ignoré (et non compté 0).
  WITH past_shifts AS (
  ...
  per_shift AS (
    SELECT ps.shift_date,
      (SELECT
         ...
         END
       FROM per_submission psub WHERE psub.shift_id = ps.id) AS cscore
    FROM past_shifts ps
  )
  SELECT
