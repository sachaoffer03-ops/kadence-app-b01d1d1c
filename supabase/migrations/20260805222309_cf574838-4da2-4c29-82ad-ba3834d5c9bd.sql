...
ALTER TABLE public.ai_planning_settings
  ADD COLUMN IF NOT EXISTS score_start_date date NOT NULL DEFAULT '2026-07-01';
...
BEGIN
  ...
  -- Ponctualité : shift pointé => score selon le retard.
  -- Shift terminé, publié et NON pointé depuis la date de début du scoring => 0.
  -- Avant cette date => ignoré.
  WITH last_shifts AS (
  ...
  -- Check-lists : shift terminé et publié.
  -- Aucune soumission alors que le studio a des check-lists actives
  -- et que le shift est postérieur à la date de début => 0. Sinon ignoré.
  WITH past_shifts AS (
